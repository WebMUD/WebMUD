...
export function mapEdit(server: Server) {
  ...
  server.commands.addCommand({
      ...
      use(argv: string[]) {
        ...
        if (owner) {
          ...
          if (!ownerID) return server.error(`Could not find entity by the name of ${owner}`);
          server.gamestate.move(result, ownerID);
          server.info(`Created prop: ${name} in ${server.gamestate.nameOf(ownerID)}`);
        } else {
  ...
  server.commands.addCommand({
    ...
    use(argv: string[]) {
        ...
        if (!firstID) return server.error(`Unable to find a room by the name of ${first}`);
        if (!secondID) return server.error(`Unable to find a room by the name of ${second}`);
